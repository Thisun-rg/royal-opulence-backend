...
"WHERE r.room.id = :roomId AND r.status IN ('PENDING','CONFIRMED','CHECKED_IN') " +
"AND (:start < r.checkOut AND :end > r.checkIn)")
boolean existsConflict(@Param("roomId") Long roomId, @Param("start") LocalDate start, @Param("end") LocalDate end);
